...
const handleDynamicCommandCommand = require('../../src/commands/Edit Commands/handleDynamicCommandCommand');

describe('handleDynamicCommandCommand', () => {
  let mockServices;
  let mockContext;
  ...
  describe('metadata', () => {
    it('should have correct metadata properties', () => {
      expect(handleDynamicCommandCommand.requiredRole).toBe('MODERATOR');
      expect(handleDynamicCommandCommand.description).toBe('Manage dynamic commands');
      expect(handleDynamicCommandCommand.example).toBe('add props');
      expect(handleDynamicCommandCommand.hidden).toBe(false);
    });
  });

  describe('add command', () => {
    it('should create a new dynamic command', async () => {
      ...
      const result = await handleDynamicCommandCommand({
    ...
    it('should reject if command already exists as dynamic command', async () => {
      fs.existsSync.mockReturnValue(true);
      fs.readFileSync.mockImplementation((filePath) => {
        if (filePath.includes('chat.json')) {
          return JSON.stringify({ props: { messages: [], pictures: [] } });
