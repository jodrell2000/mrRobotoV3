...
/**
 * Build custom data for OpenChat messages
...
/**
 * Build message payload for OpenChat API
...
async function sendMessage ( payload ) {
    ...
    const executeRequest = async () => {
        try {
            ...
            logger.error( `[OpenChat API] sendMessage error - Status: ${ error.response?.status }, Data: ${ JSON.stringify( error.response?.data, null, 2 ) }` );
            throw error;
        }
    };
...
async function joinChatGroup ( roomId ) {
    const executeRequest = async () => {
        const url = `${ BASE_URL }/v3/groups/${ roomId }/members`;
        // logger.debug( `[OpenChat API] joinChatGroup - URL: ${ url }` );
        ...
        // logger.debug( `[OpenChat API] joinChatGroup - Request data: ${ JSON.stringify( requestData ) }` );
        ...
        try {
            ...
            if ( error.response?.status === 417 &&
                ...
                logger.info( `✅ [OpenChat API] Bot is already a member of group ${ roomId }` );
...
async function leaveChatGroup ( roomId ) {
    const executeRequest = async () => {
        const url = `${ BASE_URL }/v3/groups/${ roomId }/members/${ config.BOT_UID }`;
        ...
        try {
            ...
            if ( error.response?.status === 404 || error.response?.status === 400 ) {
                logger.info( `✅ [OpenChat API] Bot is not a member of group ${ roomId }` );
                return {
                    status: 200,
...
/**
 * Fetch messages from OpenChat API with retry logic
...
async function fetchMessages ( endpoint, queryParams = [] ) {
    const executeRequest = async () => {
        ...
        if ( endpoint.includes( '?' ) ) {
            ...
            // logger.debug( `🔍 [OpenChat API] fetchMessages - Parsed endpoint path: ${ endpointPath }` );
            // logger.debug( `🔍 [OpenChat API] fetchMessages - Parsed endpoint params: ${JSON.stringify(endpointParams)}` );
        }

        // Combine endpoint params with additional params
        const allParams = [ ...endpointParams, ...queryParams ];
        // logger.debug( `🔍 [OpenChat API] fetchMessages - All combined params: ${JSON.stringify(allParams)}` );

        const url = buildUrl( BASE_URL, [ endpointPath ], allParams );

        // logger.debug( `🔍 [OpenChat API] fetchMessages - Final URL: ${ url }` );
        // logger.debug( `🔍 [OpenChat API] fetchMessages - Query params: ${ JSON.stringify( allParams ) }` );

        const response = await apiClient.get( url );

        // logger.debug( `🔍 [OpenChat API] fetchMessages - Response status: ${ response.status }` );
        // logger.debug( `🔍 [OpenChat API] fetchMessages - Response data count: ${ response.data?.data?.length || 0 }` );

        // if ( response.data?.data?.length > 0 ) {
        //     const messages = response.data.data;
        //     logger.debug( `🔍 [OpenChat API] fetchMessages - First message ID: ${ messages[ 0 ]?.id }` );
        //     logger.debug( `🔍 [OpenChat API] fetchMessages - Last message ID: ${ messages[ messages.length - 1 ]?.id }` );
        //     logger.debug( `🔍 [OpenChat API] fetchMessages - Message senders: ${ messages.map( m => m.sender?.uid || 'unknown' ).join( ', ' ) }` );
        // } else {
        //     logger.debug( `🔍 [OpenChat API] fetchMessages - No messages returned` );
        // }
    ...
    if ( retryService ) {
        try {
            ...
            if ( retryService.isCircuitOpen && retryService.isCircuitOpen( 'openchat-fetchMessages' ) ) {
                logger.error( '❌ [OpenChat API] fetchMessages - Circuit breaker is OPEN, skipping request' );
                const error = new Error( 'Circuit breaker is OPEN for openchat-fetchMessages' );
            ...
            return await retryService.executeWithRetry(
                executeRequest,
                { maxRetries: 3 },
                'openchat-fetchMessages'
            );
        } catch ( error ) {
            logger.error( `❌ [OpenChat API] fetchMessages - Error: ${ error.message }` );
            logger.error( `❌ [OpenChat API] fetchMessages - Error status: ${ error.response?.status }` );
            logger.error( `❌ [OpenChat API] fetchMessages - Error response: ${ JSON.stringify( error.response?.data, null, 2 ) }` );
            logger.error( `❌ [OpenChat API] fetchMessages - Error stack: ${ error.stack }` );

            // Check if this is an authorization error (user not in group)
            const errorCode = error.response?.data?.error?.code;
            const status = error.response?.status;
            if ( status === 401 && errorCode === 'ERR_GROUP_NOT_JOINED' ) {
                logger.warn( '⚠️ [OpenChat API] Bot is no longer a member of the group - triggering reconnect' );
