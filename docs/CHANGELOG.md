...
|---------|------|---------|---------|| **[1.4.1](changelog/1.4.1.md)** | 2026-06-30 | 🤖 Mistral LLM Backend & Chat Pagination Fix | Added Mistral AI as alternative ML backend with automatic fallback chain. Multi-backend switching with `!switchllm` command. Fixed critical chat pagination bug (messages stuck in 50-message loop) by ensuring message tracking updates on every fetch. |  | **[1.3.0](changelog/1.3.0.md)** | 2026-06-16 | 🚀 Per-User Private Message Backoff | Refactored private message fetching to parallel execution with per-user dynamic backoff (1s→10s). Fixed timeout detection and removed overall batch timeout to allow backoff escalation. Reduces fetch time from 6.5s+ to 1-2s. |
