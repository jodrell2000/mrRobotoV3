...
|---------|------|---------|---------|
| **[1.5.0](changelog/1.5.0.md)** | 2026-09-04 | 🌐 Site Independence Architecture | Major refactoring decoupling bot from ttfm-socket library through adapter pattern. Multi-site support foundation with pluggable SocketAdapter and ApiAdapter. Abstracts socket and REST API calls to single integration points, enabling future multi-site support without code duplication. All phases tested and validated. |
| **[1.4.2](changelog/1.4.2.md)** | 2026-08-29 | 👋 DJ Self-Removal Scheduling (Escortme) | New `!escortme` command allows DJs to schedule automatic removal after their song finishes. Full phase-based implementation with 117 passing tests. Includes flag persistence, automatic cleanup on DJ leave/room disconnect, multi-DJ queue support, and comprehensive error handling. |
...
| **[0.9.8_beta](changelog/0.9.8_beta.md)** | 2026-03-14 | � AFK Monitor & Auto-Removal, Moderator Commands, Theme Tokens | `!afkMonitor` management command with position-aware deferred removal; `!mod` with `listUsers`, `remove`, `skip`; `!say`; owner-settable `{theme}` / `{readTheme}` tokens; per-user welcome messages with `!editWelcome` |
