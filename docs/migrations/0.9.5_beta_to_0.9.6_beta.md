...
The transition from Gemini to Gemma models represents a strategic shift toward sustainable, free-tier AI usage. However, Gemma models are considerably simpler and less sophisticated than their Gemini counterparts. They require **significantly more explicit instructions** to produce consistent, high-quality responses. Think of it as the difference between giving detailed, step-by-step instructions to a competent employee versus a new intern—the simpler model needs much more guidance.
...
### 3. Delete the SQLite Database (only required if migrating from 0.9.)
...
⚠️ **NOTE:** If applying this using the `!edit` command, ensure newlines are preserved and quotes are not escaped. Use the format below instead:
...
**Key Changes:**
- ✅ Explicit callout of Role
- ✅ not using vauge descriptions like Hangout
- ✅ Clear mandate about embodying vs. describing personality
- ✅ Removed abstract phrasing; using concrete states
...
**Key Changes:**
- ✅ Added explicit variety requirements (MUST NOT reuse adjectives, etc.)
- ✅ Clear "FORBIDDEN" section
- ✅ Explicit context section with all available tokens
- ✅ Strong language (MUST, STRICT) instead of suggestions
...
**Key Changes:**
- ✅ Added "Task" header for clarity
- ✅ Explicit "Do NOT use history as template" at the start
- ✅ Enumerated opening options (4 specific choices)
- ✅ Numbered execution steps (1-3) instead of vague guidance
- ✅ Guidance on adjectives (descriptive, not just "vibe")
- ✅ Hard constraint on word count
...
1. **Check Bot Initialization**
   ...
   - Should see: `🤖 [MachineLearningService] Loaded X available text generation models`
...
2. Restart the application—it will work with Gemini models (if your API key permits)
